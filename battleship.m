...
while
  ...
  clc  %clears the screen to start a new game
  ...
  for shiplength=1:1:5 %loop will generate 4 more ships
    ...
    if orientation == 1 %horizontal
      while true
        trial_x=round(rand*(length(grid)-shiplength))+1;  %determines x co-ordinate
        ...
        grid(trial_x,trial_y)=shiplength;
          for i = 1:1:shiplength-1
            ...
            while true
              trial_y=round(rand*(length(grid)-shiplength))+1;     %determines x co-ordinate
              trial_x=round(rand*9)+1;       %determines y co-ordinate
              % place ship
              counter = 0;
              for i = 1:1:shiplength-1
          ...
          gameover=
          'n';
          misses=0;   %stores the number of misses
          hits=0;     %stores the number of hits
          while gameover=='n'
            userokay=
            'n';
          while userokay=='n'
        ...
        if grid(user_x, user_y)==0
          ...
          misses=misses+1;          %counts the number of misses
